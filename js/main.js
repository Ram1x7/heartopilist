...
` : ""}
  ...
  " aria-pressed="${authData[c.name] ? "true" : "false"}" aria-label="${displayName(c)} ${T("aria_auth_label","認証マスターにする")}">
    ${icon(authData[c.name] ? "medal" : "medalOutline", {size:16})}
  </button>
` : ""}
...
// shareBtn/shareModal/shareCanvas周りの配線・drawShareCard()等は
// js/share-card.js に分離済み（loadScriptOnce()は他機能でも使うためこちらに残す）
...
document.getElementById("lastUpdate").textContent =
  T("last_update_label","最終更新") + " 2026/09/14";
