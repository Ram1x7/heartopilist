...
function openBuildCropStage(reset){
  ...
  requestAnimationFrame(() => {
    ...
    // 画像全体を縮小して収められるよう、ズームスライダーの下限は「画像全体が
    // ちょうど収まる縮小率（contain）」を100%(cover)基準の割合で動的に設定する
    // （画像・切り抜き枠の縦横比の組み合わせによって毎回変わるため固定値にできない）
    const containScale = Math.min(vw / frontImage.naturalWidth, vh / frontImage.naturalHeight);
    const zoomSlider = document.getElementById("buildCropZoomSlider");
    zoomSlider.min = String(Math.max(1, Math.floor(containScale / cropCoverScale * 100)));
    ...
    zoomSlider.value = String(Math.max(Number(zoomSlider.min), Math.round(cropZoom * 100)));
...
// ズームで画像をcover基準より縮小できるようにしたことで、片方または両方の
// 軸で画像の表示サイズ（disp）が切り抜き枠（viewport）より小さくなる場合が
// ある。その軸は従来通りの「枠をはみ出す前提」のドラッグ範囲クランプではなく、
// 枠の中央に固定する（はみ出さない軸は動かす意味が無いため）
function clampCropAxis(disp, viewportSize, pos){
  if(disp <= viewportSize) return (viewportSize - disp) / 2;
  return Math.min(0, Math.max(viewportSize - disp, pos));
}
...
function applyBuildCropTransform(){
  ...
  cropLeft = clampCropAxis(dispW, vw, cropLeft);
  cropTop = clampCropAxis(dispH, vh, cropTop);
